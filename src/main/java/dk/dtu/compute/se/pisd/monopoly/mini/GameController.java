 ...
 * {@link dk.dtu.compute.se.pisd.monopoly.mini.model.Game} need to be made
 * through this controller, since the GUI does not follow the MVC pattern. For
 * fully implementing the game, it will be necessary to add more of these basic
 * actions in this class.
 * 
 * The action methods of the
 * {@link dk.dtu.compute.se.pisd.monopoly.mini.model.Space} and the
 * {@link dk.dtu.compute.se.pisd.monopoly.mini.model.Card} can be implemented
...
public class GameController {
	...
	public void chooseNewOrLoadGame() {
		...
		try {
			...
			e.printStackTrace();
			if (gui.getUserLeftButtonPressed("Der skette en fejl: "+e, "Prøv igen", "Afslut")) {
				chooseNewOrLoadGame();
			}
			else {
				System.exit(1);
	...
	private void createPlayers() throws SQLException {
		...
		ArrayList<Color> colorList = new ArrayList<Color>(
				Arrays.asList(Color.BLUE, Color.RED, Color.GREEN, Color.YELLOW, Color.BLACK, Color.WHITE));
		ArrayList<String> colorString = new ArrayList<String>(
				Arrays.asList("Blå", "Rød", "Grøn", "Gul", "Sort", "Hvid"));
		...
		view.playerUpdate();
		dao.create(game);
		String p = "Spillet er oprettet med GameID: " + game.getGameID();
		gui.showMessage(p);
	...
	public void play() {
		...
		while (!terminated) {
			...
			if (countActive == 1) {
				gui.showMessage("Spiller " + winner.getName() + " har vundet med " + winner.getBalance() + "kr.");
				break;
				...
				// bankrupt)
				gui.showMessage("Alle spillerne er gået fallit.");
				break;
			...
			if (current == 0) {
				String selection = gui.getUserSelection("Runden er slut. Vil I fortsætte med at spille?", "ja", "nej");
				if (selection.equals("nej")) {
					...
					if (selection.equals("ja")) {
						...
						gui.showMessage(
								"Vinderen er: " + ranking.get(0).toString() + ". Spillet lukkes når du trykker ok.");
					}
					terminated = true;
				}
			}
	...
	/**
	 * This method implements a activity of a single move of the given player. It
	 * throws a
	 * {@link dk.dtu.compute.se.pisd.monopoly.mini.model.exceptions.PlayerBrokeException}
		...
		do {
			...
			if (player.isInPrison()) {
				...
				if (castDouble) {
					...
					String selection = gui.getUserSelection(player.getName() + ", du kastede ikke to ens, hvad vil du?",
							"Betal kaution", "Brug frikort", "Vent til næste tur");
					...
					if (player.isInPrison() && player.getPrisonTime() >= game.MAX_PRISON_TIME) {
						gui.showMessage(player.getName()
								+ " har været i fængselet for længe, du tvinges nu til at betale kaution af "
								+ game.JAIL_BAIL_PRICE + "kr.");
						paymentToBank(player, game.JAIL_BAIL_PRICE);
						if (!player.isBroke()) {
							player.setInPrison(false);
						}
			...
			if (castDouble) {
				doublesCount++;
				if (doublesCount > 2) {
					gui.showMessage("Spiller " + player.getName()
							+ " har kastet to ens tre gange nu og havner derfor i fængslet!");
					gotoJail(player);
					return;
				}
			}
	...
	public void moveToSpace(Player player, Space space) throws PlayerBrokeException {
		...
		gui.showMessage(
				"Spiller " + player.getName() + " lander på " + space.getIndex() + ": " + space.getName() + ".");

		// If the space is the IncomeTax one, we give 2 choices for payment
		if (space instanceof IncomeTax) {
			String msg = player.getName()
					+ ", du skal betale indkomstskat af 4.000kr. eller 10% af alle dine værdier (huse, hoteller, grunde og balance)";
	...
	public void obtainCash(Player player, int amount) {
		...
		} while (!gui.getUserLeftButtonPressed("Du har optjent " + earned + "kr. hvad vil du?", "Fortsæt",
				"Sælg mere"));
	...
	public void offerToBuy(Property property, Player player) throws PlayerBrokeException {
		...
		String choice = gui.getUserSelection("Spiller " + player.getName() + ": Vil du købe " + property.getName()
				+ " for " + property.getCost() + "kr.?", "ja", "nej");
	...
	public void payment(Player payer, int amount, Player receiver) throws PlayerBrokeException {
		...
		gui.showMessage(
				"Spiller " + payer.getName() + " betaler " + amount + "kr. til spiller " + receiver.getName() + ".");
	...
	public void auction(Property property) {
		...
		while (bidders.size() > 1) {
			for (Player player : bidders.toArray(new Player[0])) {
				...
				if (bidders.size() <= 1) {
					...
					currentBid = gui.getUserInteger(
							"Indtast dit bud. Det skal være minimum " + minNextBid + "kr. højere end " + currentBid,
							currentBid + minNextBid, Integer.MAX_VALUE);
				} else {
		...
		if (bidders.size() > 0) {
			...
			gui.showMessage("Tillykke " + auctionWinner.getName() + ", du har vundet auktionen! " + "Du har købt: "
					+ property.getName() + " for " + currentBid + "kr.");
	...
	public void playerBrokeTo(Player brokePlayer, Player benificiary) {
		...
		gui.showMessage("Spiller " + brokePlayer.getName() + "gik fallit og overfører alle hans " + "likvider til "
				+ benificiary.getName());
	}
	...
	private void tradeSellHouse(Player seller) {
		...
		RealEstate chosenProperty = (RealEstate) pickProperty(undevelopableProperties,
				"Vælg den grund du vil uudvikle.");
	...
	/**
	 * Handles the transaction of the ownership of a property from the seller to the
	 * buyer at the given prices.
	 * 
	 * @param buyer
	 *            player buying the property
	 * @param property
	 *            the asset
	 * @param price
	 *            the agreed price
	 * @param seller
	 *            player selling the property
	 */
	 ...
	 * 
	 * @param properties
	 *            Map with the properties and names as key value
	 * @param msg
	 *            the dialog message shown to the player
	 ...
	 * 
	 * @param properties
	 *            the players owned properties
	 * @param colorGroup
	 *            the color of the group
	 * @return <code>true</code> if colorgroup is complete, if not then
	 *         <code>false</code>.
	 */
	...
	/**
	 * Method used to get a ranked list of the players based on their assets.
	 * Highest score at first index.
	 * 
	 * @param players
	 *            a list of the players to be ranked.
